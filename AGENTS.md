...
> ⚠️ **下次发版版本号**：当前 `0.2.11`，0.2.x 已累积 6+ 个 patch（0.2.6 ~ 0.2.11），玻璃质感功能已稳定，**下次发版直接跳到 `0.3.0`**（或 `0.3.0-beta.0` 预发布），不再发 0.2.x。0.2.11 之后又累积了用户可见的新特性（玻璃预设档位、设置页自动检查更新、关闭主题时还原原官方偏好、更新检查 5 分钟缓存），更坐实 0.3.0 的 minor 定位。
